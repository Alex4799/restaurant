                            ...
                            <div class="d-flex align-items-center w-100 me-4">
                                <select name="store_id" id="" class="form-control">
                                    ...
                                    @foreach ($store as $item)
                                        <option value="{{$item->id}}" @if ($item->id==request('store_id')) selected @endif>{{$item->name}} ({{ $item->shop_name }})</option>
                                    @endforeach
                                </select>
                    ...
                    <table id="table-list2">
                        ...
                        <tbody class="py-4">
                            @foreach ($transfer as $item)
                                <tr>
                                    <td>{{$item->id}}</td>
                                    <td>{{$item->send_store_name}}</td>
                                    <td>{{$item->receive_store_name}}</td>
                                    ...
                                    <td>
                                        <a href="{{route('admin#transferView',$item->id)}}" class="text-bg-primary px-2 py-1 rounded" title="view more">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{route('admin#transferCreate')}}" method="POST" class="mt-1">
                            @csrf
                            <div class="form-group mb-4">
                                <label for="" class="mb-1 fw-semibold">Send Store Name</label>
                                <div class="d-flex align-items-center">
                                    <select name="send_store_id" id="" class="form-control">
                                        <option value="" disabled selected>Choose a store</option>
                                        @foreach ($store as $item)
                                            <option value="{{$item->id}}">{{$item->name}} ({{ $item->shop_name }})</option>
                                        @endforeach
                                    </select>
                                    <i class="fa-solid fa-chevron-down" id="method-drop-icon2"></i>
                                </div>
                            ...
                            <div class="form-group mb-4">
                                <label for="" class="mb-1 fw-semibold">Receive Store Name</label>
                                <div class="d-flex align-items-center">
                                    <select name="receive_store_id" id="" class="form-control">
                                        <option value="" disabled selected>Choose a store</option>
                                        @foreach ($store as $item)
                                            <option value="{{$item->id}}">{{$item->name}} ({{ $item->shop_name }})</option>
                                        @endforeach
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input type="number" name="additional_fees" class="form-control" value="0" min="0">
                            </div>
                            <div class="form-group mb-4">
